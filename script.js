...
function showOldPassword(){
	var text = document.getElementById('oldPassword');
	if(text.type === "password"){
		text.type = "text";
	}else{
		text.type = "password";
...
function showNewPassword(){
	var text = document.getElementById('newPassword');
	if(text.type === "password"){
		text.type = "text";
	}else{
		text.type = "password";
...
function checkOldPass(){
	...
	if(oldPass != pass){
		document.getElementById('message_Ps').style.color = 'red';
		document.getElementById('message_Ps').innerHTML = '<i class="fa-sharp fa-solid fa-xmark"></i> Mật khẩu không khớp với mật khẩu cũ';
	}else if(oldPass == pass){
		document.getElementById('message_Ps').style.color = 'green';
		document.getElementById('message_Ps').innerHTML = '<i class="fa-solid fa-check"></i> Mật khẩu trùng với mật khẩu cũ';
	}if(oldPass == ""){
		document.getElementById('message_Ps').innerHTML = "";
	}
}
...
function checkNewPass(){
	...
	let pass = getPassword();
	if(newPass == pass){
		document.getElementById('message_nPs').style.color = 'red';
		document.getElementById('message_nPs').innerHTML = '<i class="fa-sharp fa-solid fa-xmark"></i> Mật khẩu không được trùng với mật khẩu cũ';
	}
	if(newPass != pass){
		document.getElementById('message_nPs').style.color = 'green';
		document.getElementById('message_nPs').innerHTML = '<i class="fa-solid fa-check"></i> Mật khẩu mới hợp lệ';
	}
}

function checkRTNewPW(){
	let retypePass = document.getElementById('retypePassword').value;
	let newPass = document.getElementById('newPassword').value;
	if(retypePass == newPass){
		document.getElementById('message_rnPs').style.color = 'green';
		document.getElementById('message_rnPs').innerHTML = '<i class="fa-solid fa-check"></i> Mật khẩu mới hợp lệ';
	}
	if(retypePass != newPass){
		document.getElementById('message_rnPs').style.color = 'red';
		document.getElementById('message_rnPs').innerHTML = '<i class="fa-sharp fa-solid fa-xmark"></i> Mật khẩu xác nhận không đúng';
	}
}
...
function login(){
	...
	if(getPassword() == undefined || getMail() == undefined){
		alert("Tài khoản không tồn tại, vui lòng tạo tài khoản");
		window.open("dangky.html");
	}
	else{
...
function readValue(){
	...
	var thupass=/(?=^.{8,}$)(?=.*\d)(?=.*[!@#$%^&*]+)(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/;
	if(firstName.length>0 && lastName.length>0 && sdt.test(phoneNumber)==true && Email.test(mailAddress)==true && thupass.test(password)==true && retypepassword ==password ){
...
function showmessage(){
	...
	else if(localStorage.getItem("mail") != email.value ){
		alert("Email hoặc mật khẩu không chính xác");
		return false;
	} 
}
